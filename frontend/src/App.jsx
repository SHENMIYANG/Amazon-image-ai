...
function buildCompletedImageState(image = {}, generatedImage = {}, fallbackPlan = {}) {
  const executionPlan = normalizeImagePlan({
    ...
    currentImageProductUsage: generatedImage.currentImageProductUsage || fallbackPlan.currentImageProductUsage || image.currentImageProductUsage,
  ...
  return {
    ...
    databasePlanId: executionPlan.databasePlanId || image.databasePlanId,
    databasePlanVersionId: executionPlan.databasePlanVersionId || image.databasePlanVersionId,
...
function getPageRoute(pathname = window.location.pathname) {
  ...
  if (activityMatch) return { name: 'activity-detail', workspaceId: activityMatch[1] }
  if (normalizedPath === '/activity') return { name: 'activity' }
  if (normalizedPath === '/members') return { name: 'members' }
...
function App() {
  ...
  const handleListingChange = (field, value) => {
    ...
    if (field === 'marketplace') {
      setListing((prev) => {
        ...
        const shouldSyncLanguage = !prev.imageLanguageTouched || !prev.imageLanguage || prev.imageLanguage === previousDefaultLanguage
  ...
  const syncStrategyTranslations = async (plansOverride = null, targetPlanTaskKeys = null) => {
    ...
    try {
      const listingSnapshot = buildListingPayload(listing, {
        includeGenerationSettings: true
      })
      ...
      setListing((prev) => ({
        ...
        imagePlans: (prev.imagePlans || []).map((currentPlan) => {
          ...
          const contentChangedDuringSave = String(currentPlan.strategyContent || '').trim() !== String(translatedPlan.strategyContent || '').trim()
  ...
  const handleGenerate = async () => {
    ...
    const plansNeedingSavedPrompt = allPlans.filter((plan) => plan.taskType !== 'main' && (plan.promptDirty || !String(plan.promptEn || '').trim()))
    ...
    const listingSnapshot = {
      ...buildListingPayload(listing, { includeGenerationSettings: true }),
      _meta: listing._meta
    }
    const newTask = {
      id: taskId,
              ...
              if (task.id === taskId) {
                return {
                  ...
                  images: task.images.map((img) =>
                    img.imageId === plan.id
                      ? {
                          ...buildCompletedImageState(img, generatedImage, plan),
      ...
      setTasks((prev) =>
        prev.map((task) => {
          ...
          return {
            ...task,
            status: 'failed',
            images: task.images.map((img) => (img.status === 'completed' ? img : { ...img, status: 'failed', error: error.message }))
          }
        })
      )
  ...
  const handleRegenerate = async (task, imageIndex, options = {}) => {
    ...
    const providedReferenceImageUrls = Array.isArray(options.referenceImageUrls) ? options.referenceImageUrls.filter(Boolean) : []
    ...
    try {
      ...
      const primaryReferenceImageUrl = task.primaryReferenceImageUrl || getPrimaryReferenceImageUrl(task.referenceImages || [])
      const additionalReferenceImages = await uploadReferenceFiles(referenceFiles, `图${image.imageId}追加参考图上传接口`)
      const regenerationReferenceImages = [...(task.referenceImages || []), ...providedReferenceImageUrls, ...additionalReferenceImages].filter(
        (url, index, source) => url && source.indexOf(url) === index
      )
      const generatedImage = await requestGeneratedImage({
        listing: taskListing,
        plan: planToUse,
  ...
  const handleContinue = async (task) => {
    ...
    const primaryReferenceImageUrl = task.primaryReferenceImageUrl || getPrimaryReferenceImageUrl(referenceImages)
  ...
  const handleDownload = async (imageUrl, filename, requestedResolution) => {
    const resolveDownloadUrl = (url) => {
      ...
      if (['localhost', '127.0.0.1'].includes(parsedUrl.hostname) && !['localhost', '127.0.0.1'].includes(window.location.hostname)) {
  ...
  if (authLoading) {
    return <main className="auth-loading">正在检查登录状态...</main>
  }
  ...
  if (authEnabled && pageRoute.name === 'members' && currentUser?.role === 'ADMIN') {
    return <MembersPage currentUser={currentUser} onBack={() => navigateTo('/')} />
  }

  if (authEnabled && (pageRoute.name === 'activity' || pageRoute.name === 'activity-detail') && currentUser) {
    return <ActivityPage currentUser={currentUser} workspaceId={pageRoute.workspaceId || ''} />
  }
          ...
          {authEnabled && currentUser && (
            <div className="header-account">
              <div className="header-account-name">
                <strong>{currentUser.displayName || currentUser.loginName}</strong>
                <span>{currentUser.role === 'ADMIN' ? '管理员' : currentUser.role === 'OPERATOR' ? '运营' : '美工'}</span>
              </div>
            ...
            <section className="section workspace-panel action-panel">
              ...
              <div className="stack-panel-group action-panel-group">
                <div className="sub-panel compact-sub-panel">
                  <ResolutionSelector selected={selectedResolution} onChange={setSelectedResolution} />
                </div>
